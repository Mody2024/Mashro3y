...
function fmtCur(n,cur="USD"){
  ...
  try{return new Intl.NumberFormat(c.l,{style:"currency",currency:cur,maximumFractionDigits:2,minimumFractionDigits:0}).format(n||0);}
  catch{return `${c.s}${(n||0).toLocaleString()}`;}
}
...
function fmtDateShort(d){if(!d)return"—";try{return new Date(d).toLocaleDateString("en-GB",{day:"2-digit",month:"short"});}catch{return d;}}
...
en:{
  // App
  app:"MASHRO3Y",tagline:"Enterprise ERP v8.1",
  // Nav
  ...
  reconcile:"Reconcile",bankStatement:"Bank Statement",
  costCenter:"Cost Center",project:"Project",
  workSchedule:"Work Schedule",shift:"Shift",
},
ar:{
  app:"مشروعي",tagline:"نظام ERP v8.1",
  ...
  reconcile:"مطابقة",bankStatement:"كشف بنكي",
...
function Toast({toasts}){
  ...
  return React.createElement("div",{style:{position:"fixed",top:16,right:16,zIndex:9999,display:"flex",flexDirection:"column",gap:8,pointerEvents:"none",maxWidth:320}},
...
function ClientProfile({client,onBack}){
  const {t,tr,fmt,cI,cR,cCRM,openModal,setClis,acId}=useApp();
  const [tab,setTab]=React.useState("overview");
  ...
  const avgPayDays=cInvs.filter(i=>i.paid&&i.paidDate&&i.date).reduce((acc,i,_,arr)=>{
    return acc+daysBetween(i.date,i.paidDate)/arr.length;
  },0);

  return(
    <div style={{minHeight:"100vh",background:t.bg}}>
      <div style={{background:t.bg2,borderBottom:`1px solid ${t.bd}`,padding:"20px 24px"}}>
        <button onClick={onBack} style={{display:"flex",alignItems:"center",gap:6,background:"none",border:"none",color:t.tx3,cursor:"pointer",fontSize:13,fontWeight:600,marginBottom:16}}>← Back to Clients</button>
        <div style={{display:"flex",gap:20,alignItems:"flex-start",flexWrap:"wrap"}}>
          <Avatar name={client.name} photo={client.photo} size={80}/>
          <div style={{flex:1}}>
            <div style={{display:"flex",gap:10,alignItems:"center",flexWrap:"wrap",marginBottom:6}}>
              <h1 style={{fontSize:24,fontWeight:900,color:t.tx,margin:0}}>{client.name}</h1>
              {overdueInvs.length>0&&<Badge status="Overdue"/>}
              {client.tags?.map((tag,i)=><span key={i} style={{padding:"3px 10px",borderRadius:20,background:`${t.a}20`,color:t.a,fontSize:11,fontWeight:700}}>{tag}</span>)}
            </div>
            <div style={{fontSize:15,color:t.tx3,marginBottom:2}}>{client.position} {client.company?"at "+client.company:""}</div>
            <div style={{fontSize:13,color:t.tx4}}>{client.address}</div>
            <div style={{display:"flex",gap:14,marginTop:10,flexWrap:"wrap"}}>
              {client.phone&&<a href={`tel:${client.phone}`} style={{fontSize:12,color:t.a,textDecoration:"none",display:"flex",alignItems:"center",gap:4}}>📞 {client.phone}</a>}
              {client.email&&<a href={`mailto:${client.email}`} style={{fontSize:12,color:t.a,textDecoration:"none",display:"flex",alignItems:"center",gap:4}}>✉ {client.email}</a>}
              {client.website&&<a href={"https://"+client.website} target="_blank" style={{fontSize:12,color:t.a,textDecoration:"none",display:"flex",alignItems:"center",gap:4}}>🌐 {client.website}</a>}
            </div>
          </div>
          <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
            <Btn onClick={()=>openModal("invoice",{clientId:client.id,clientName:client.name,clientEmail:client.email,clientAddress:client.address})} variant="primary" icon="🧾">New Invoice</Btn>
            <Btn onClick={()=>openModal("client",client)} variant="secondary" icon="✏">Edit</Btn>
            <Btn onClick={()=>{
              const lines=["CLIENT STATEMENT","","Client: "+client.name,"Date: "+fmtDate(today()),"","INVOICES:",""];
              cInvs.forEach(i=>lines.push(`${i.num} | ${fmtDate(i.date)} | ${fmt(calcInvTotal(i))} | ${invStatus(i)}`));
              ...
              win.document.write("<pre style='font-family:monospace;padding:40px;font-size:14px'>"+lines.join("\n")+"</pre>");
              win.print();
            }} variant="ghost" icon="📄">Statement</Btn>
          </div>
        </div>
      </div>
      ...
      <div style={{padding:"16px 24px 0"}}>
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(160px,1fr))",gap:12,marginBottom:20}}>
          {[
            ...
            {label:"Total Invoiced",value:fmt(totalInvoiced),color:t.a,icon:"🧾"},
            {label:"Total Paid",value:fmt(totalPaid),color:"#10B981",icon:"✅"},
            {label:"Outstanding",value:fmt(outstanding),color:outstanding>0?"#EF4444":"#10B981",icon:"⏳"},
            {label:"Credit Limit",value:fmt(client.creditLimit||0),color:"#8B5CF6",icon:"💳"},
            {label:"Avg Pay Time",value:Math.round(avgPayDays)+" days",color:"#F59E0B",icon:"📅"},
          ].map((s,i)=>(
            <div key={i} style={{background:t.bg2,borderRadius:12,padding:"14px 16px",border:`1px solid ${t.bd}`}}>
              <div style={{display:"flex",justifyContent:"space-between",marginBottom:6}}>
                <span style={{fontSize:11,color:t.tx4,fontWeight:600,textTransform:"uppercase"}}>{s.label}</span>
        ...
        {outstanding>0&&outstanding>(client.creditLimit||0)&&(
          <div style={{background:"#EF444415",border:"1px solid #EF444440",borderRadius:12,padding:"12px 16px",marginBottom:16,display:"flex",gap:10,alignItems:"center"}}>
            <span style={{fontSize:20}}>⚠️</span>
            <div>
              <div style={{fontWeight:700,color:"#EF4444"}}>Credit Limit Exceeded</div>
              <div style={{fontSize:12,color:t.tx3}}>Outstanding {fmt(outstanding)} exceeds credit limit of {fmt(client.creditLimit||0)}</div>
            </div>
          </div>
        )}
        ...
        <Tabs tabs={[
          {id:"overview",label:"Profile",icon:"👤"},
          {id:"invoices",label:"Invoices",icon:"🧾",count:cInvs.length},
          {id:"statement",label:"Statement",icon:"📄"},
          {id:"crm",label:"CRM Activity",icon:"🤝",count:cLeads.length},
        ]} active={tab} onChange={setTab}/>
      </div>
      ...
      <div style={{padding:"0 24px 24px"}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16}}>
            <Section title="Contact Information" icon="📞">
              <InfoGrid cols={2} items={[
                ...
                {label:"Position",value:client.position},
            ...
            <Section title="Financial Summary" icon="💰">
              <InfoGrid cols={1} items={[
                ...
                {label:"Credit Limit",value:fmt(client.creditLimit||0),color:t.a},
                {label:"Total Invoiced",value:fmt(totalInvoiced),color:t.a},
                {label:"Total Paid",value:fmt(totalPaid),color:"#10B981"},
                {label:"Outstanding Balance",value:fmt(outstanding),color:outstanding>0?"#EF4444":"#10B981"},
                {label:"Number of Invoices",value:cInvs.length+" invoices"},
                {label:"Overdue Invoices",value:overdueInvs.length+" invoices",color:overdueInvs.length>0?"#EF4444":undefined},
                {label:"Average Payment Time",value:Math.round(avgPayDays)+" days"},
              ]}/>
            </Section>
          </div>
        )}
        ...
        {tab==="invoices"&&(
          <Card>
            <Table
              cols={[
                {key:"num",label:"Invoice #",render:(v,row)=><span style={{fontWeight:700,color:t.a,cursor:"pointer"}}>{v}</span>},
                {key:"date",label:"Date",render:v=><span style={{color:t.tx3,fontSize:12}}>{fmtDate(v)}</span>},
                {key:"dueDate",label:"Due",render:(v,row)=>{const st=invStatus(row);return<span style={{color:st==="Overdue"?"#EF4444":t.tx3,fontSize:12}}>{fmtDate(v)}</span>;}},
                ...
                {key:"status",label:"Status",render:(v,row)=><Badge status={invStatus(row)}/>},
              ]}
        ...
        {tab==="statement"&&(
          <div>
            <div style={{display:"flex",justifyContent:"flex-end",marginBottom:16}}>
              <Btn variant="ghost" icon="🖨" onClick={()=>window.print()}>Print Statement</Btn>
            </div>
            <div style={{background:t.bg2,borderRadius:16,padding:32,border:`1px solid ${t.bd}`}}>
              <div style={{display:"flex",justifyContent:"space-between",marginBottom:24}}>
                <div>
                  <div style={{fontSize:22,fontWeight:900,color:t.tx,marginBottom:4}}>CLIENT STATEMENT</div>
                  <div style={{fontSize:13,color:t.tx3}}>As of {fmtDate(today())}</div>
                </div>
                <div style={{textAlign:"right"}}>
                  <div style={{fontWeight:700,color:t.tx}}>{client.name}</div>
                  <div style={{fontSize:12,color:t.tx4}}>{client.company}</div>
                  <div style={{fontSize:12,color:t.tx4}}>{client.address}</div>
                </div>
              </div>
                ...
                <thead>
                  <tr style={{background:t.bg3}}>
                    {["Date","Ref","Description","Debit","Credit","Balance"].map((h,i)=>(
                      <th key={i} style={{padding:"10px 12px",textAlign:i>2?"right":"left",fontWeight:700,color:t.tx3,borderBottom:`2px solid ${t.bd}`,fontSize:11}}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                    ...
                    return(
                      <React.Fragment key={inv.id}>
                        <tr style={{borderBottom:`1px solid ${t.bd}`}}>
                          <td style={{padding:"9px 12px",color:t.tx3,fontSize:12}}>{fmtDate(inv.date)}</td>
                          <td style={{padding:"9px 12px",fontWeight:600,color:t.a}}>{inv.num}</td>
                          <td style={{padding:"9px 12px",color:t.tx}}>Invoice</td>
                          <td style={{padding:"9px 12px",textAlign:"right",fontWeight:700}}>{fmt(total)}</td>
                          <td style={{padding:"9px 12px",textAlign:"right",color:t.tx4}}>—</td>
                          <td style={{padding:"9px 12px",textAlign:"right",fontWeight:700,color:runBal>0?"#EF4444":"#10B981"}}>{fmt(runBal)}</td>
                        </tr>
                        {inv.paid>0&&<tr style={{borderBottom:`1px solid ${t.bd}`,background:t.bg3+"60"}}>
                          <td style={{padding:"9px 12px",color:t.tx3,fontSize:12}}>{fmtDate(inv.paidDate)}</td>
                          <td style={{padding:"9px 12px",color:t.tx4,fontSize:12}}>{inv.payRef||"—"}</td>
                          <td style={{padding:"9px 12px",color:"#10B981"}}>Payment Received ({inv.payMethod})</td>
                          <td style={{padding:"9px 12px",textAlign:"right",color:t.tx4}}>—</td>
                          <td style={{padding:"9px 12px",textAlign:"right",color:"#10B981",fontWeight:700}}>{fmt(inv.paid)}</td>
              ...
              <div style={{display:"flex",justifyContent:"flex-end"}}>
                <div style={{width:260}}>
                  {[["Total Invoiced",fmt(totalInvoiced)],["Total Paid",fmt(totalPaid)]].map(([l,v],i)=>(
                    <div key={i} style={{display:"flex",justifyContent:"space-between",padding:"7px 0",borderBottom:`1px solid ${t.bd}`}}>
                      <span style={{color:t.tx3}}>{l}</span><span style={{fontWeight:700}}>{v}</span>
                    </div>
                  ))}
                  <div style={{display:"flex",justifyContent:"space-between",padding:"12px 14px",background:outstanding>0?"#EF444420":"#10B98120",borderRadius:10,marginTop:8}}>
                    <span style={{fontWeight:800,color:t.tx}}>BALANCE DUE</span>
                    <span style={{fontWeight:900,fontSize:18,color:outstanding>0?"#EF4444":"#10B981"}}>{fmt(outstanding)}</span>
                  </div>
                </div>
              </div>
            </div>
        ...
        {tab==="crm"&&(
          <div>
            {cLeads.length===0?(
              <div style={{textAlign:"center",padding:48,color:t.tx4}}>
                <div style={{fontSize:32,marginBottom:8}}>🤝</div>
                <div>No CRM activity for this client</div>
                <Btn onClick={()=>openModal("crmLead",{email:client.email,company:client.company,name:client.name})} variant="primary" style={{marginTop:16}}>Add to Pipeline</Btn>
              </div>
            ):(
              cLeads.map(lead=>(
                <div key={lead.id} style={{background:t.bg3,borderRadius:12,padding:16,marginBottom:12}}>
                  <div style={{display:"flex",justifyContent:"space-between",marginBottom:8}}>
                    <div style={{fontWeight:700,color:t.tx}}>{lead.company} — {lead.name}</div>
                    <Badge status={lead.stage}/>
                  </div>
                  <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:10,fontSize:12}}>
                    <div><span style={{color:t.tx4}}>Deal Value: </span><b style={{color:t.a}}>{fmt(lead.dealValue)}</b></div>
                    <div><span style={{color:t.tx4}}>Probability: </span><b>{lead.probability}%</b></div>
                    <div><span style={{color:t.tx4}}>Assigned: </span><span>{lead.assignedTo}</span></div>
                  </div>
                  {lead.notes&&<div style={{fontSize:12,color:t.tx4,marginTop:8}}>📝 {lead.notes}</div>}
                </div>
              ))
            )}
          </div>
...
function ClientsPage(){
  const {t,tr,fmt,cC,setClis,cI,openModal}=useApp();
  const [search,setSearch]=React.useState("");
  const [selectedClient,setSelectedClient]=React.useState(null);

  if(selectedClient)return<ClientProfile client={selectedClient} onBack={()=>setSelectedClient(null)}/>;

  const filtered=React.useMemo(()=>cC.filter(c=>{
    const s=search.toLowerCase();
    return!s||(c.name||"").toLowerCase().includes(s)||(c.company||"").toLowerCase().includes(s)||(c.email||"").toLowerCase().includes(s);
  }),[cC,search]);
  ...
  return(
    <div style={{padding:24}}>
      <PageHeader title="Clients" icon="👥" subtitle={`${cC.length} clients · Outstanding: ${fmt(totalOutstanding)}`}
        actions={<Btn onClick={()=>openModal("client",{})} variant="primary" icon="＋">Add Client</Btn>}
      />
          ...
          return(
            <div key={client.id} onClick={()=>setSelectedClient(client)}
              ...
              <div style={{display:"flex",gap:12,alignItems:"flex-start",marginBottom:14}}>
                <Avatar name={client.name} photo={client.photo} size={48}/>
                <div style={{flex:1,minWidth:0}}>
                  <div style={{fontSize:15,fontWeight:800,color:t.tx,marginBottom:1}}>{client.name}</div>
              ...
              {client.tags?.length>0&&<div style={{display:"flex",gap:4,flexWrap:"wrap",marginBottom:10}}>
                {client.tags.map((tag,i)=><span key={i} style={{padding:"2px 8px",borderRadius:20,background:`${t.a}15`,color:t.a,fontSize:10,fontWeight:700}}>{tag}</span>)}
              </div>}
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:12}}>
                <div style={{background:t.bg3,borderRadius:8,padding:"8px 10px"}}>
                  <div style={{fontSize:10,color:t.tx4}}>Outstanding</div>
                  <div style={{fontSize:13,fontWeight:700,color:outstanding>0?"#EF4444":"#10B981"}}>{fmt(outstanding)}</div>
                </div>
                <div style={{background:t.bg3,borderRadius:8,padding:"8px 10px"}}>
                  <div style={{fontSize:10,color:t.tx4}}>Invoices</div>
                  <div style={{fontSize:13,fontWeight:700,color:t.tx}}>{cInvs.length} total</div>
                </div>
              </div>
              <div style={{display:"flex",gap:6}}>
                {client.phone&&<a href={`tel:${client.phone}`} onClick={e=>e.stopPropagation()} style={{flex:1,padding:"6px",borderRadius:8,background:t.bg3,color:t.tx3,fontSize:11,textAlign:"center",textDecoration:"none",fontWeight:600}}>📞 Call</a>}
                {client.email&&<a href={`mailto:${client.email}`} onClick={e=>e.stopPropagation()} style={{flex:1,padding:"6px",borderRadius:8,background:t.bg3,color:t.tx3,fontSize:11,textAlign:"center",textDecoration:"none",fontWeight:600}}>✉ Email</a>}
                <button onClick={e=>{e.stopPropagation();openModal("invoice",{clientId:client.id,clientName:client.name,clientEmail:client.email,clientAddress:client.address});}} style={{flex:1,padding:"6px",borderRadius:8,background:`${t.a}20`,color:t.a,border:"none",fontSize:11,cursor:"pointer",fontWeight:700}}>🧾 Invoice</button>
              </div>
            </div>
          );
        })}
...
function ProcurementPage(){
  const {t,tr,fmt,cSup,cPO,setSups,setPos,openModal}=useApp();
      ...
      {tab==="suppliers"&&<div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(280px,1fr))",gap:14}}>
        {cSup.map(sup=>(
          <Card key={sup.id} style={{padding:20}}>
            ...
            <div style={{display:"flex",gap:8}}>
              {sup.email&&<a href={`mailto:${sup.email}`} style={{flex:1,padding:"6px",borderRadius:8,background:t.bg3,color:t.tx3,textAlign:"center",textDecoration:"none",fontSize:11,fontWeight:600}}>✉ Email</a>}
  ...
  return(
    <div style={{padding:24,maxWidth:720,margin:"0 auto"}}>
      ...
      <div style={box}>
        ...
        <div style={{fontSize:13,color:t.tx4,lineHeight:1.8}}>Version 8.1 Enterprise · Multi-company ERP · Full HR & Payroll · CRM Pipeline · Invoice & Quotes · Procurement · Cold Storage · AI Assistant · Bilingual EN/AR</div>
      </div>
    </div>
  );
}
...
function AllModals(){
  ...
  const save=()=>{
    ...
    if(modal==="revenue"){
      ...
      if(!r("name")){showToast(tr.fillRequired,"error");return;}
      const item={id:isEdit?r("id"):uid(),name:r("name"),currency:r("currency")||"AED",logo:(r("name")||"").slice(0,2).toUpperCase(),tax:+r("tax")||5,color:r("color")||"#3B82F6",type:r("type")||"Trading",address:r("address")||"",taxNum:r("taxNum")||"",phone:r("phone")||"",email:r("email")||"",website:r("website")||"",archived:false,fiscalYearStart:"01",workingDays:["Mon","Tue","Wed","Thu","Fri"],dailyWorkHours:9,leaveEntitlement:30};
...
function Sidebar({mod,setMod}){
  const{t,tr,companies,acId,setAcId}=useApp();
  const co=companies.find(c=>c.id===acId)||companies[0]||{};
  const groups=[
    {label:"Overview",items:[{id:"dashboard",icon:"⬡",label:"Dashboard"}]},
    {label:"Finance",items:[{id:"revenues",icon:"💰",label:tr.revenue},{id:"expenses",icon:"📉",label:tr.expenses},{id:"invoices",icon:"🧾",label:tr.invoices},{id:"quotes",icon:"📋",label:tr.quotes},{id:"vault",icon:"🏦",label:tr.vault},{id:"budget",icon:"🎯",label:tr.budget}]},
    {label:"Business",items:[{id:"clients",icon:"👥",label:tr.clients},{id:"crm",icon:"🎯",label:"CRM"},{id:"products",icon:"📦",label:tr.products},{id:"procurement",icon:"🛒",label:tr.procurement}]},
    {label:"HR",items:[{id:"workers",icon:"👷",label:tr.workers}]},
    {label:"Operations",items:[{id:"utilities",icon:"⚡",label:tr.utilities},{id:"storage",icon:"❄",label:tr.storage}]},
    {label:"Tools",items:[{id:"reports",icon:"📊",label:tr.reports},{id:"ai",icon:"🤖",label:"AI Assistant"}]},
    {label:"Admin",items:[{id:"companies",icon:"🏢",label:tr.companies},{id:"settings",icon:"⚙",label:tr.settings}]},
  ];
  return(
    <div className="sidebar" style={{width:220,background:t.bg2,borderRight:`1px solid ${t.bd}`,height:"100vh",position:"fixed",top:0,left:0,zIndex:100,flexDirection:"column",overflow:"hidden"}}>
      <div style={{padding:"14px 14px 10px",borderBottom:`1px solid ${t.bd}`,flexShrink:0}}>
        <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:12}}>
          ...
          <div><div style={{fontSize:14,fontWeight:900,color:t.tx}}>MASHRO3Y</div><div style={{fontSize:9,color:t.tx4,letterSpacing:.5}}>ENTERPRISE v8.1</div></div>
        </div>
      ...
      <div style={{padding:"10px 12px",borderTop:`1px solid ${t.bd}`,flexShrink:0}}>
        <div style={{display:"flex",alignItems:"center",gap:8}}>
          ...
          <div style={{flex:1,minWidth:0}}>
            ...
            <div style={{fontSize:10,color:t.tx4}}>{co.currency} · {co.tax}% tax</div>
          </div>
        </div>
      </div>
    </div>
...
function BottomNav({mod,setMod}){
  const{t,tr}=useApp();
  const items=[{id:"dashboard",icon:"⬡",label:"Home"},{id:"invoices",icon:"🧾",label:"Invoices"},{id:"revenues",icon:"💰",label:"Revenue"},{id:"workers",icon:"👷",label:"Workers"},{id:"clients",icon:"👥",label:"Clients"},{id:"settings",icon:"⚙",label:"More"}];
  return(
    <div className="bottom-nav" style={{position:"fixed",bottom:0,left:0,right:0,background:t.bg2,borderTop:`1px solid ${t.bd}`,zIndex:100,paddingBottom:"env(safe-area-inset-bottom,0px)"}}>
      {items.map(item=>(
        <button key={item.id} onClick={()=>setMod(item.id)} style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",gap:2,padding:"8px 4px",border:"none",background:mod===item.id?t.a+"15":"transparent",color:mod===item.id?t.a:t.tx4,cursor:"pointer",fontSize:18}}>
          <span>{item.icon}</span>
...
function Header({mod,setMod}){
  const{t,tr,companies,acId}=useApp();
  const co=companies.find(c=>c.id===acId)||companies[0]||{};
  const pages={dashboard:"Dashboard",revenues:"Revenue",expenses:"Expenses",invoices:"Invoices",quotes:"Quotes",clients:"Clients",crm:"CRM Pipeline",products:"Products",procurement:"Procurement",workers:"Workers",utilities:"Utilities",storage:"Cold Storage",reports:"Reports",ai:"AI Assistant",companies:"Companies",settings:"Settings",vault:"Vault",budget:"Budget"};
  return(
    <div style={{height:52,background:t.bg2,borderBottom:`1px solid ${t.bd}`,display:"flex",alignItems:"center",padding:"0 20px",gap:12,position:"sticky",top:0,zIndex:50,flexShrink:0}}>
      <div style={{fontSize:14,fontWeight:700,color:t.tx,flex:1}}>{pages[mod]||mod}</div>
      <div style={{fontSize:11,color:t.tx4,fontWeight:600}}>{co.name} · {co.currency}</div>
...
function App(){
  ...
  const fmt=useCallback(n=>fmtCur(n,co.currency||"AED"),[co]);
  ...
  const ctx={
    ...
    showToast,openModal,closeModal,modal,modalData,setMod,
  };
